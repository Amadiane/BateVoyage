...
function ModalAjoutPelerinsVehicule({ vehiculeId, placesRestantes, onFermer, onAjoute }) {
  ...
  useEffect(() => {
    pelerinService.lister().then(({ data }) => {
      setPelerins(data.filter((p) => !p.vehicule || String(p.vehicule) !== String(vehiculeId)));
      setChargement(false);
    });
  }, [vehiculeId]);
  ...
  const valider = async () => {
    ...
    setErreur("");
    try {
      await vehiculeService.affecterPelerins(vehiculeId, selectionnes);
      onAjoute();
      onFermer();
    } catch (err) {
    ...
    <div className={styles.superposition} onClick={onFermer}>
      <div className={styles.panneau} onClick={(e) => e.stopPropagation()}>
        <div className={styles.entete}>
          <h2 className={styles.titre}>{t("ajouter_pelerins_vehicule")}</h2>
...
export default ModalAjoutPelerinsVehicule;