...
function ModalAjoutPelerinsProgramme({ programmeId, onFermer, onAjoute }) {
  ...
  useEffect(() => {
    pelerinService.lister().then(({ data }) => {
      setPelerins(data.filter((p) => String(p.programme) !== String(programmeId)));
      setChargement(false);
    });
  }, [programmeId]);
  ...
  const valider = async () => {
    ...
    setEnvoi(true);
    try {
      await programmeService.affecterPelerins(programmeId, selectionnes);
      onAjoute();
      onFermer();
    } finally {
      setEnvoi(false);
    ...
    <div className={styles.superposition} onClick={onFermer}>
      <div className={styles.panneau} onClick={(e) => e.stopPropagation()}>
        <div className={styles.entete}>
          <h2 className={styles.titre}>{t("ajouter_pelerins_activite")}</h2>
        ...
        <div className={styles.liste}>
          ...
          {!chargement && pelerinsFiltres.map((p) => (
            <label key={p.id} className={styles.ligne}>
              <input type="checkbox" checked={selectionnes.includes(p.id)} onChange={() => basculer(p.id)} />
...
export default ModalAjoutPelerinsProgramme;