...
function PageGroupesModule({ basePath }) {
  ...
  const ouvrirModification = (g, e) => {
    ...
    setValeurs({
      nom: g.nom, sensVol, volId, encadreur: g.encadreur || "",
      responsable_medical: g.responsable_medical || "", capacite_max: g.capacite_max || "", notes: g.notes || "",
    });
    setErreur("");
    setModalOuverte(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const donnees = {
        nom: valeurs.nom, encadreur: valeurs.encadreur, responsable_medical: valeurs.responsable_medical,
  ...
  const confirmerSuppression = async () => {
    await groupeService.supprimer(aSupprimer.id);
    setASupprimer(null);
    charger();
  };
            ...
            <div key={g.id} className={styles.carte} onClick={() => navigate(`${basePath}/groupes/${g.id}`)}>
              ...
              <div className={styles.bandeau}>
                ...
                <div className={styles.actions} onClick={(e) => e.stopPropagation()}>
                  ...
                  <button onClick={() => setASupprimer(g)} title={t("supprimer")} className={styles.boutonSupprimer}><Trash2 size={13} /></button>
                </div>
              </div>
              ...
              {tauxRemplissage !== null && (
                <div className={styles.barreProgression}>
                  <div className={`${styles.barreRemplie} ${tauxRemplissage >= 100 ? styles.barreComplete : ""}`} style={{ width: `${tauxRemplissage}%` }} />
                </div>
              )}
            </div>
          );
        ...
        <div className={styles.superposition} onClick={() => setModalOuverte(false)}>
          <div className={styles.panneau} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.formulaire}>
              <div className={styles.champ}><label>{t("nom_groupe")}</label><input value={valeurs.nom} onChange={(e) => majChamp("nom", e.target.value)} placeholder="Groupe Hajj A" /></div>

              <div className={styles.champ}>
                <label>{t("type_vol_label")}</label>
                <div className={styles.choixSensVol}>
                  <button type="button" className={valeurs.sensVol === "aller" ? styles.boutonTypeActif : styles.boutonType} onClick={() => majSensVol("aller")}>{t("vol_aller")}</button>
                  <button type="button" className={valeurs.sensVol === "retour" ? styles.boutonTypeActif : styles.boutonType} onClick={() => majSensVol("retour")}>{t("vol_retour")}</button>
                  <button type="button" className={valeurs.sensVol === "" ? styles.boutonTypeActif : styles.boutonType} onClick={() => majSensVol("")}>{t("aucun")}</button>
                </div>
              </div>
              ...
              {valeurs.sensVol && (
                <div className={styles.champ}>
                  ...
                  <select value={valeurs.volId} onChange={(e) => majChamp("volId", e.target.value)}>
                    <option value="">—</option>
                    {vols.filter((v) => v.type_vol === valeurs.sensVol).map((v) => <option key={v.id} value={v.id}>{v.compagnie} {v.numero_vol} — {v.date_vol}</option>)}
                  </select>
                </div>
              )}

              <div className={styles.champ}><label>{t("encadreur")}</label><input value={valeurs.encadreur} onChange={(e) => majChamp("encadreur", e.target.value)} placeholder={t("saisir_nom_encadreur")} /></div>
              <div className={styles.champ}><label>{t("responsable_medical")}</label><input value={valeurs.responsable_medical} onChange={(e) => majChamp("responsable_medical", e.target.value)} placeholder={t("saisir_nom_responsable_medical")} /></div>
              <div className={styles.champ}><label>{t("capacite_max")}</label><input type="number" min="1" value={valeurs.capacite_max} onChange={(e) => majChamp("capacite_max", e.target.value)} placeholder="Ex: 45" /></div>
